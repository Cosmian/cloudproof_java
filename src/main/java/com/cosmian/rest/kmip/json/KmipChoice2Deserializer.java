...
public class KmipChoice2Deserializer<E extends KmipChoice2<?, ?>> extends KmipJsonDeserializer<E> {

    private static final Logger logger = Logger.getLogger(KmipChoice2Deserializer.class.getName());

    private final Class<?> handled_type;

    public KmipChoice2Deserializer(Class<E> handled_type) {
    ...
    public E deserialize(JsonNode node, DeserializationContext context) throws IOException, JacksonException {
        ...
        for (Class<?> p_class : KmipJson.type_parameters_for_super_class(clazz, KmipChoice2.class)) {
